...
- **Core Function**: To create a Continuous Integration (CI) workflow using GitHub Actions that automatically publishes the `ddd-tools` package to the NPM registry.
...
| :-- | :---------------------------------------- | :------- | :------------- | :---------------------------------------------------------------------------------------------------------- |
| D-1 | `p1-p4.t15-npm-executable-config.task.md` | Internal | 💡 Not Started | The package must be fully configured before automating its publication.                                     |
| D-2 | `NPM_TOKEN` GitHub Secret                 | External | 💡 Not Started | A secret named `NPM_TOKEN` containing a valid NPM publish token must be created in the repository settings. |
