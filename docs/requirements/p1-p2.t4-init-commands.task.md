...
- The `ddd init` command must create a `docs/` directory in the current working directory if one does not already exist.
- The generated `docs/` directory must contain the standard DDD subdirectories: `requirements/` and `templates/`.
- The command must populate the `docs/` directory with the core methodology documents (`ddd-2.md`, `ddd-2-schema.machine.md`).
- The `docs/templates/` directory must be populated with `plan.template.md` and `task.template.md`.
- The `docs/requirements/` directory must be populated with an example `p1-example.plan.md` to guide users, following the new naming convention.
- The command should not overwrite an existing `docs/` directory unless a `--force` flag is provided.

### 2.4 Acceptance Criteria

| ID   | Criterion                                                                                            | Test Reference          |
| :--- | :--------------------------------------------------------------------------------------------------- | :---------------------- |
| AC-1 | `ddd init` creates a `docs/` directory with `requirements/` and `templates/` subdirectories.         | CLI integration test    |
| AC-2 | The `docs/` directory is populated with `ddd-2.md` and `ddd-2-schema.machine.md`.                    | File content validation |
| AC-3 | The `docs/templates/` directory contains `plan.template.md` and `task.template.md`.                  | File existence test     |
| AC-4 | The `docs/requirements/` directory contains an example `p1-example.plan.md`.                         | File existence test     |
| AC-5 | The command fails with a clear error if the `docs/` directory already exists.                        | Conflict handling test  |
| AC-6 | The command succeeds if `docs/` exists and the `--force` flag is used, overwriting previous content. | Force flag test         |
| AC-7 | The command provides clear feedback upon successful creation of the documentation structure.         | User experience test    |
...
| :--- | :-------------------------------------------------------------------- | :---------- | :--------------------- | :---------------------------------------- |
| AC-1 | `ddd init` creates the full `docs/` directory structure.              | Integration | Jest + temp filesystem | Verify all subdirs are created.           |
| AC-2 | Core documents (`ddd-2.md`, etc.) are copied correctly.               | Integration | Jest + temp filesystem | Check for file existence and content.     |
| AC-3 | Template files (`plan.template.md`, etc.) are copied correctly.       | Integration | Jest + temp filesystem | Check for file existence.                 |
| AC-4 | An example `p1-example.plan.md` is generated in `docs/requirements/`. | Integration | Jest + temp filesystem | Verify integration with TemplateGenerator |
| AC-5 | `ddd init` fails when `docs/` exists without `--force`.               | Integration | Jest + temp filesystem | Test conflict handling.                   |
| AC-6 | `ddd init --force` overwrites an existing `docs/` directory.          | Integration | Jest + temp filesystem | Test overwrite functionality.             |
| AC-7 | The command provides clear success and error feedback to the console. | Unit        | Jest + console capture | Verify user experience.                   |
