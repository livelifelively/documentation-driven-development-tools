...
| :--- | :------------------------------------------------------ | :---------- | :----------------------- | :------------------------------------------------------------------------------ |
| AC-1 | Run `ddd commit` from a valid task branch.              | Integration | Jest, `simple-git` mocks | Verify `stageAll` and `commit` are called with the correctly formatted message. |
| AC-3 | Check the task ID extraction logic.                     | Unit        | Jest                     | Test the regex or string manipulation that parses the branch name.              |
| AC-5 | Run `ddd commit` from a non-task branch (e.g., `main`). | Integration | Jest, `simple-git` mocks | The command should fail early with a specific error message.                    |
